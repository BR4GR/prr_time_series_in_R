...
# Funktionen
## for lopp to fill up all na with values
...
# hiermit gruppiere ich zinsätze monatlich
monthly_df <- combined_df %>% 
  mutate(month = floor_date(date, "month")) %>%
  group_by(month) %>%summarise(
    sum_rate = sum(change_rate_prozent, na.rm = TRUE),
    sum_closing = sum(change_cp_prozent, na.rm = TRUE)
  )


ggplot(monthly_df, aes(x = month)) +
  geom_line(aes(y = sum_rate, color = "Leitzins")) +
  geom_line(aes(y = sum_closing, color = "S&P500")) +
...
Diese Kennzahlen liefern einen umfassenden Überblick über die zentrale Tendenz und die Streuung des Leitzinses und bieten wertvolle Einblicke in die geldpolitische Landschaft während des betrachteten Zeitraums.
...
```
Time Series Modelling with VAR
```{r}
adf_test_closing_price <- adf.test(diff_combined_df$diff_closing_price)
adf_test_rate <- adf.test(diff_combined_df$diff_rate)

adf_test_closing_price
adf_test_rate

# Autokorrelation der Residuen überprüfen
residuals_var <- residuals(var_model)
acf_residuals_closing_price <- acf(residuals_var$diff_closing_price, main = "ACF of Residuals (diff_closing_price)")
acf_residuals_rate <- acf(residuals_var$diff_rate, main = "ACF of Residuals (diff_rate)")

# Differenzieren der Daten außerhalb von mutate
diff_closing_price <- diff(log(combined_df$closing_price))
diff_rate <- diff(combined_df$rate)

# Erstellung eines neuen DataFrames mit differenzierten Daten
diff_combined_df <- combined_df[-1, ] %>%
  mutate(diff_closing_price = diff_closing_price,
         diff_rate = diff_rate)

# Überprüfen der Struktur von diff_combined_df
str(diff_combined_df)

# Sicherstellen, dass die Spalten korrekt benannt sind
colnames(diff_combined_df)

# VAR-Modell erstellen
var_data <- diff_combined_df %>%
  dplyr::select(diff_closing_price, diff_rate)

var_model <- VAR(var_data, p = 1)
summary(var_model)

# Prognosen mit dem VAR-Modell
var_forecast <- predict(var_model, n.ahead = 12)
var_forecast_df <- data.frame(
  time = seq(as.Date(max(combined_df$date)) + 1, by = "month", length.out = 12),
  diff_closing_price = var_forecast$fcst$diff_closing_price[, "fcst"],
  diff_rate = var_forecast$fcst$diff_rate[, "fcst"]
)

# Überprüfen der Vorhersagedaten
print(var_forecast_df)

# Plot VAR-Prognosen
ggplot(var_forecast_df, aes(x = time)) +
  geom_line(aes(y = diff_closing_price, color = "S&P 500")) +
  geom_line(aes(y = diff_rate, color = "Leitzins")) +
  labs(title = "VAR-Prognosen für S&P 500 und Leitzins",
       x = "Zeit",
       y = "Differenzierter Wert",
       color = "Variable") +
  theme_minimal()
```
